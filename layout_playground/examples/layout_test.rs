...
use layout_playground::ParquetLayoutStrategy;
...
struct ParquetLayoutTestStrategy;

impl LayoutTestStrategy for ParquetLayoutTestStrategy {
    fn name(&self) -> &'static str {
        "parquet"
    }

    fn description(&self) -> &'static str {
        "Parquet layout strategy with compression and optimization"
    }

    fn create_strategy(&self) -> ArcRef<dyn LayoutStrategyTrait> {
        ParquetLayoutStrategy::with_executor(Arc::new(LocalExecutor))
    }
}
...
impl LayoutStrategyRegistry {
    fn new() -> Self {
        ...
        let test_strategies: Vec<Box<dyn LayoutTestStrategy>> = vec![
            ...
            Box::new(ParquetLayoutTestStrategy),
        ];
...
impl LayoutTester {
    ...
    async fn write_with_strategy(
        ...
        match strategy.name() {
            "flat" | "chunked" | "struct" | "vortex_default" | "parquet" => {
...
async fn main() -> VortexResult<()> {
    // Initialize tracing with TRACE level logging
    use tracing_subscriber::EnvFilter;

    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("trace")); // Default to TRACE level
